...
const getDepartments = async (): Promise<{ name: string; value: number }[]> => {
  ...
  const getRolesByDepartment = async (departmentId: number): Promise<{ name: string; value: number }[]> => {
...
const getEmployees = async (): Promise<{ name: string; value: number | null }[]> => {
...
const updateEmployee = async () => {
  ...
  const employees = await getEmployees();
  const { id, first_name, last_name, role_id, manager_id }: { id: string, first_name: string, last_name: string, role_id: string, manager_id: string } = await inquirer.prompt([
    ...
    { type: 'list', name: 'role_id', message: 'Select new role (leave blank to skip):', choices: roles, when: (answers): boolean => !answers.role_id },
